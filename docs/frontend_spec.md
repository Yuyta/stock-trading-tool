...
* ユーザーインターフェースの中核であり、以下の機能を持ちます：
  ...
  * **分析フォーム**: 銘柄コード/シンボル（企業名からのオートコンプリート検索対応）、時間軸、トレードスタイルの入力。
  ...
  * **退会機能**: ユーザーアカウントの完全削除。確認ダイアログ表示後に `/api/user` (DELETE) を呼び出し、全データを消去します。
...
* **企業名表示**: 銘柄コードに加えて、企業名をリスト内に表示。
* **検索**: 銘柄コードおよび企業名によるフィルタリング。
* **削除**: 不要な履歴を `/api/history` (DELETE) により削除。
* **ソート**: 分析日時、銘柄名での並び替え。
...
* `AnalysisResult`, `AnalysisHistory`, `User`, `MacroResult`, `SearchResult` 等のインターフェース定義。
